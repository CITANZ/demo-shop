...
<form class="column is-4" method="post" @submit.prevent="add_to_cart()">
    ...
    <div class="content">
        <strong>Price</strong>: {{product.special_price ? product.special_price.toDollar() : product.price.toDollar()}} <template v-if="product.special_price"><strike>{{product.price.toDollar()}}</strike>, {{product.special_rate}}%</template>
    </div>
    <div class="field">
        <div class="control">
